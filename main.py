...
def add_expense():
    ...
    if not description or not amount:
        messagebox.showwarning("تنبيه", "من فضلك املأ جميع الخانات!")
        return

    try:
        amount = float(amount)
    except ValueError:
        messagebox.showerror("خطأ", "برجاء إدخال رقم صحيح في خانة المبلغ")
        return
    ...
    messagebox.showinfo("نجاح", f"تم إضافة {description} بنجاح!")
    desc_entry.delete(0, tk.END)
    amount_entry.delete(0, tk.END)

# --- دالة الرسم البياني (Matplotlib) ---
def show_report():
    categories_data = {}
    ...
    try:
        ...
        if not categories_data:
            messagebox.showinfo("تقرير", "لا توجد بيانات لعرضها!")
            return
        ...
        plt.title("توزيع المصاريف حسب الفئة")
        plt.show()

    except FileNotFoundError:
        messagebox.showerror("خطأ", "ملف البيانات غير موجود!")
...
tk.Label(root, text="المبلغ (Amount):", font=("Arial", 10)).pack(pady=5)
amount_entry = tk.Entry(root, width=40)
amount_entry.pack()

tk.Label(root, text="الفئة (Category):", font=("Arial", 10)).pack(pady=5)
...
# ربط زرار التقرير بالدالة الجديدة (command=show_report)
